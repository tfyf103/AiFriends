...
Thank you for helping improve AiFriends. The project is both a real full-stack AI application and a step-by-step learning repository, so contributions should preserve two qualities at the same time:
...
Mock mode lets contributors test JWT, Django, Friend, SSE, Vue, and Message persistence without spending external AI credits.
...
GitHub Actions repeats the main checks in a clean environment and also builds the learning Docker image.
...
- `README.md` — entry point and current capabilities;
...
- `labs/` — chapter exercises affected by the change.
...
- [ ] Learner-facing docs are updated when behavior or architecture changed.
